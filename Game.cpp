...
#include "../GameObjects/Player/Bot/Bot.h"
#include "../GameObjects/Player/Craussie/Craussie.h"
#include "../GameObjects/Player/AntWarrior/AntWarrior.h"
...
void Game::loop(Curses::Key /*= Crs::Key::ESC*/)
{
	...
	while(outer_state_ == State::Finish)
	{
		...
		Curses::mvwprintw(win, 2, 2, "Thanks for playing X-Ant.");
...
void Game::end()
{
	...
	while(prompt != NULL)
	{
		...
		Curses::mvwprintw(prompt, 3, 1, "Don't be sad, though, you did manage to reach ");
		Curses::mvwprintw(prompt, 4, 1, "the floor %d in the dungeon, so yeah, good job", dungeon_.floor());
		Curses::mvwprintw(prompt, 5, 1, "                                              ");
		Curses::mvwprintw(prompt, 6, 1, "Your HI-SCORE is saved, so don't be scared    ");
		Curses::mvwprintw(prompt, 7, 1, "of going through the dungeon again, mate.     ");
		Curses::mvwprintw(prompt, 8, 1, "                                              ");
...
void Game::titleScreen()
{
	...
	while(win != NULL)
	{
		...
		Curses::mvwprintw(win,  4, 0, "                 ::. ###::::'#######:'##:::. ##: ## ## ##:::: ##::::");
		Curses::mvwprintw(win,  5, 0, "                 :: ## ##:::........: #########: ##. ####:::: ##::::");
		Curses::mvwprintw(win,  6, 0, "                 : ##:. ##::::::::::: ##.... ##: ##:. ###:::: ##::::");
		Curses::mvwprintw(win,  7, 0, "                  ##:::. ##:::::::::: ##:::: ##: ##::. ##:::: ##::::");
		Curses::mvwprintw(win,  8, 0, "                 ..:::::..:::::::::::..:::::..::..::::..:::::..:::::");
		Curses::mvwprintw(win, 16, 30, "~ Sort of roguelike? I hope? ~");
		...
		Curses::mvwprintw(win, 20, 25, "So, who are you? Funny face you got there.");
		...
		Curses::wattron(win, COLOR_PAIR(static_cast<int>(GameObject::Color::Cyan_Black)));
		Curses::mvwprintw(win, 32, 20, "<Move and attack with arrows, exit the game by Esc>");
		Curses::wattroff(win, COLOR_PAIR(static_cast<int>(GameObject::Color::Cyan_Black)));

		Curses::mvwprintw(win, 39, 0, "Made by X-Corp. and CHAOS Industries, 2024.");
		Curses::wrefresh(win);
		...
		if(key != -1)
		{
			...
			{
				Curses::delwin(win);
				player_.reset(new Craussie(dungeon_)); //Craussie
				Curses::flash();
				return;
			}
			else if((key == 'b') || (key == 'B'))
			{
				Curses::delwin(win);
				player_.reset(new Bot(dungeon_)); //Bot
				Curses::flash();
				return;
			}
			else if((key == 'a') || (key == 'A'))
			{
				Curses::delwin(win);
				player_.reset(new AntWarrior(dungeon_)); //Ant
				Curses::flash();
				return;
			}
		}
...
bool Game::saveData()
{
	std::ofstream file("data.croissant", std::ios::out | std::ios::out | std::ios::binary);
...
bool Game::loadData()
{
	std::ifstream file("data.croissant", std::ios::in | std::ios::binary);
