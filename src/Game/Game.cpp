...
void Game::titleScreen()
{
	...
	while(win != NULL)
	{
		...
		Curses::mvwprintw(win, 23, 36, "[C]rossie");
		...
		Curses::mvwprintw(win, 23, 56, "[A]ntWarrior");
		...
		if(key != -1)
		{
			...
			{
				Curses::delwin(win);
				player_.reset(new Elf(dungeon_)); //Crossie
				Curses::flash();
				return;
			}
			else if((key == 'b') || (key == 'B'))
			{
				Curses::delwin(win);
				player_.reset(new Orc(dungeon_)); //Bot
				Curses::flash();
				return;
			}
			else if((key == 'a') || (key == 'A'))
			{
				Curses::delwin(win);
				player_.reset(new Dwarf(dungeon_)); //Ant
				Curses::flash();
				return;
			}
		}
